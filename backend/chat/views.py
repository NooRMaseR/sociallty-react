...
class UserMessagesApi(AsyncAPIView):
    ...
    @swagger_auto_schema(
        operation_description="Get messages between the current user and the user with the given ID.",
        responses={
            200: "Success",
            404: "Not Found"
        },
    ...
    async def get(self, request: Request, user_id: int) -> Response:
        ...
        user_to_chat, page_num = await asyncio.gather(
            sync_to_async(lambda: get_object_or_404(SocialUser, id=user_id))(),
        ...
        messages = await sync_to_async(lambda: (
            ...
            .filter(Q(from_user=user_to_chat, to_user=request.user) | Q(from_user=request.user, to_user__id=user_id))
            .order_by('-sent_at')
            )
        )()
